...
# Modifications: added sentence_entropy_info field to RewardInput and propagated
# it from non_tensor_batch to reward function inputs in both sequential and
# batch reward manager mixins.
...
class RewardInput(TypedDict):
    ...
    response_length: int
    ground_truth: str
    sentence_entropy_info: Optional[dict]
...
class SequentialFunctionRewardManagerMixin:
    ...
    def compute_reward_sequential(self, data: DataProto) -> Tuple[torch.Tensor, dict[str, list[float]]]:
        ...
        for i in range(len(data)):
            ...
            # Add sentence_entropy_info if available
            if "sentence_entropy_info" in data.non_tensor_batch:
                reward_input["sentence_entropy_info"] = data.non_tensor_batch["sentence_entropy_info"][i]
            else:
                reward_input["sentence_entropy_info"] = None
